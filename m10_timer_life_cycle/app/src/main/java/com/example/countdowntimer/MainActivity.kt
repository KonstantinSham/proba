...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        slider.addOnChangeListener { _, value, fromUser ->
            if (fromUser) {
                counter = value.toInt()
                textTimer.text = counter.toString()
            }
        }
        ...
        savedInstanceState?.let { bundle ->
            if (bundle.getBoolean("RunTimer")) {
                startTimer()
            }
            counter = bundle.getInt("Counter")
            step = bundle.getInt("Progress")
        }
    }

    override fun onSaveInstanceState(outState: Bundle) {
        outState.putBoolean("RunTimer", isRunning)
        outState.putInt("Counter", current)
        outState.putInt("Progress", step)
